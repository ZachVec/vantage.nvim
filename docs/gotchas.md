...
`on_close` handler; `pick_plain` (the wizard steps and `:Vantage prompt`)
wraps its `on_choice`, because snacks' own `ui_select` shim owns `on_close`
there. Real switches and Tool-row creations already end in terminal mode via
`Client.focus`'s `show` (`startinsert`) and skip the re-entry.
