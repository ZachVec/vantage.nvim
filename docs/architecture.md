...
- The **Frontend** operates exactly one **Client** (the single `:terminal`), re-targeted with each switch, and selects through the **Picker**, a pluggable interface of its own: a picker implementation (`native` / `fzf-lua` / `snacks`) is chosen via `setup { picker = … }` and resolved through `Picker.get()`, used for the preview-capable selections (the Agent list, the kill list, and the `files` / `buffers` Actions). Implementations own rendering (including per-Agent pane previews); shared item construction lives in `picker/items.lua`, and pane previews go through the Backend (`capture_pane`) so the Frontend never touches tmux directly. The `files` / `buffers` Actions delegate to the picker backend's own file/buffer sources and exist only on `fzf-lua` and `snacks` (`native` exposes no `pick_files` / `pick_buffers`, so `:Vantage prompt` hides the Actions there). Plain selections with nothing to preview — the Tool and Group choice during Agent creation, and the `:Vantage prompt` name list — use `vim.ui.select` via `select.lua`.
