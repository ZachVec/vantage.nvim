...
- The **Frontend** operates exactly one **Client** (the single `:terminal`), re-targeted with each switch, and selects through the **Picker**, a pluggable interface of its own: a picker implementation (`native` / `fzf-lua` / `snacks`) is chosen via `setup { picker = … }` and resolved through `Picker.get()`, used only for the preview-capable selections (the Agent list and the kill list). Implementations own rendering (including per-Agent pane previews); shared item construction lives in `picker/items.lua`, and pane previews go through the Backend (`capture_pane`) so the Frontend never touches tmux directly. The Picker owns every Vantage selection: the Agent-creation wizard (Tool then Group) and `:Vantage prompt` pick through its plain-select form (`pick_plain`), which each implementation renders with its own engine — only `native` calls `vim.ui.select`, the environment's renderer by definition. Light Yes/No confirmations use Neovim's built-in confirm dialog.
